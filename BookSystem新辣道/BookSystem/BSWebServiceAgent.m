...
- (NSString *)serviceUrl:(NSString *)api arg:(NSString *)arg{
    ...
    if ([strAPI isEqualToString:@"validateCouponCode"]||[strAPI isEqualToString:@"consumerCouponCode"]) {
        str = [NSString stringWithFormat:@"http://192.168.0.110:8080/ChoiceWebService/services/UnitePaymentService?"];
    }else
    {
...
//- (NSString *)serviceUrlarg:(NSString *)arg{
//    
//    NSArray *docPaths =  NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
//    NSString *docPath = [docPaths objectAtIndex:0];
//    NSString *settingPath = [docPath stringByAppendingPathComponent:@"ip.plist"];
...
//        [[NSDictionary dictionaryWithObject:str forKey:@"ip"] writeToFile:settingPath atomically:NO];
//    }
//    if ([strAPI isEqualToString:@"validateCouponCode"]) {
//        str = [NSString stringWithFormat:@"http://192.168.0.110:8080/ChoiceWebService/services/UnitePaymentService?"];
//    }else
//    {
//        str = [NSString stringWithFormat:@"http://%@/ChoiceWebService/services/HHTSocket?",str];
...
- (NSDictionary *)GetDataarg:(NSString *)arg {
	NSString *strUrl;
	
	strUrl = [NSString stringWithFormat:@"http://192.168.0.252:8021/autoUpdateService/webService/autoService/%@",arg];
    NSLog(@"log = %@",strUrl);
