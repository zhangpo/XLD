...
{
    if ([tfSetting.text length]>0){
        [[NSUserDefaults standardUserDefaults] setObject:tfSetting.text forKey:@"zeng"];
        [[NSUserDefaults standardUserDefaults] synchronize];
        [self performSelector:@selector(cancel)];
    }
    else{
...
- (void)finishedDownloading:(NSNumber *)pass{
    ...
    NSArray *array= [bs selectNotice];
    
    NSMutableString *FujiaName=[NSMutableString stringWithFormat:@"<html><head><meta http-equiv='content-type' content='text/html;charset=utf-8'><marquee direction=up>"];
    for (NSDictionary *dict in array) {
        [FujiaName appendFormat:@"<H1><div align='center'><FONT face=黑体 size=5>%@</font></div></H1><H1><FONT face=黑体 size=4>%@</font></H1>",[dict objectForKey:@"VTITLE"],[dict objectForKey:@"VCONTENT"]];
    }
    [FujiaName appendString:@"</marquee></script></body></html>"];

    [FujiaName writeToFile:[@"natice.html" documentPath] atomically:NO encoding:NSUTF8StringEncoding error:nil];
//    [FujiaName writeToFile: atomically:NO];
//    DianPuId
    [[NSUserDefaults standardUserDefaults] setObject:[bs selectSCODE] forKey:@"DianPuId"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    if (valid){
...
//    netAccess.delegate=self;
    ...
    NSDictionary *dict1=[bs updataHHT:dict];
    [dict release];
    if (dict1) {
        bs_dispatch_sync_on_main_thread(^{
            [self.navigationController popViewControllerAnimated:YES];
